...
interface IKyberRouter {
    function getAmountsIn(uint amountIn, address[] calldata poolPath,address[] calldata path) external view returns (uint[] memory amounts);
...
interface IKyberFactory{
 function getUnamplifiedPool(address,address) external view returns(address);
}
...
contract RecipeV3 is IRecipe, Ownable {
    ...
    IKyberRouter kyberRouter = IKyberRouter(0xEaE47c5D99f7B31165a7f0c5f7E0D6afA25CFd55);
    IUniRouter sushiRouter = IUniRouter(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);
    IKyberFactory kyberFactory = IKyberFactory(0x5F1fe642060B5B9658C15721Ea22E982643c095c);
    ...
    function dexSwap(address _assetIn, address _assetOut, uint _amountOut, uint _ammIndex) public {
        ...
        //Kyber
        if(_ammIndex == 3){
            /*address poolAddress = kyberFactory.getUnamplifiedPool(_assetIn, _assetOut);
            address[] memory poolStep = new address[](1);
            poolStep[0] = poolAddress;

            IERC20(_assetIn).approve(address(kyberRouter), 0);
            IERC20(_assetIn).approve(address(kyberRouter), type(uint256).max);
            kyberRouter.swapTokensForExactTokens(_amountOut,type(uint256).max,poolStep,getRoute(_assetIn, _assetOut),address(this),block.timestamp + 1);
            return;*/
        }
    ...
    function getBestPrice(address _assetIn, address _assetOut, uint _amountOut) public returns (BestPrice memory){  
        ...
        console.log("_assetOut",_assetOut);
        console.log("sushiAmount",sushiAmount);

        //GET KYBER PRICE
        /*address poolAddress = kyberFactory.getUnamplifiedPool(_assetIn, _assetOut);
        address[] memory poolStep = new address[](1);
        poolStep[0] = poolAddress;
        try kyberRouter.getAmountsIn(_amountOut,poolStep,getRoute(_assetIn, _assetOut)) returns(uint256[] memory amounts) {
            kyberAmount = amounts[0];
        } catch {
            kyberAmount = type(uint256).max;
        }    
        if(bestPrice.price>kyberAmount){
            bestPrice.price = kyberAmount;
            bestPrice.ammIndex = 3;
        }
        console.log("kyberAmount",kyberAmount);*/
